...
- Five observation points within the same landscape: crater overlook, boulder field, Earth viewing point, a lunar module resting on the plain, and a shadowed crater hollow. The numbered buttons offer quick travel to the first three; the last two can only be reached on foot. Arriving by quick travel does not automatically record a discovery.
...
NASA 3D Resources models (`models/*.glb`) are Draco-compressed — every page that loads a GLB also loads `DRACOLoader.js` and points it at the r128 `examples/js/libs/draco/gltf/` decoder path, otherwise the geometry fails to decode and the loader silently falls back.
...
- Five discoveries — layered mesa, basalt boulder (3D specimen viewer), dune field, a parked Mars rover, degraded crater rim. The rover and crater are on foot only. Progress saves under `mzu-mars-discoveries-v1`.
...
- Terrain is cratered like the Moon but warmer gray-brown: a large central-peak crater (the rim viewpoint is a discovery), a lobate scarp — a long thrust-fault ridge from Mercury's global contraction, walkable to its crest — and a field of bright-rimmed hollows (a Mercury-exclusive terrain type). A real MESSENGER spacecraft model (NASA 3D Resources) stands in as the relay satellite honouring Mariner 10, MESSENGER, and BepiColombo; no real lander has ever touched Mercury.
...
- All five expeditions share the post-processing pipeline introduced here: each page loads the r128 `examples/js` postprocessing modules (unpkg) and renders through `EffectComposer` — `RenderPass` → `UnrealBloomPass` (per-planet strength/threshold: Moon 0.6/0.8, Mars 0.45/0.8, Venus 0.5/0.75, Mercury 0.65/0.72, Jupiter 0.5/0.74) → `GammaCorrectionShader` → `FXAAShader` → a custom cinematic pass (vignette + animated film grain). Composer use is skipped on `low` quality or when the CDN modules fail, falling back to direct rendering.
- Pages with a naked sun (Moon, Mars, Mercury) also insert `flare-pass.js` (`window.SolarFlare`) between bloom and gamma: a shared screen-space lens-flare shader that samples the rendered frame at the sun's projected position each frame, then derives an anamorphic beam, vertical spike, halo ring, and a chromatic ghost chain from the measured brightness — so terrain or haze occluding the sun genuinely dims the flare and an off-screen sun casts none. World-space sprites that model real atmosphere (the Moon's horizon glow, Mars's blue scattering halo) are kept; they are not lens artifacts.
- On Jupiter the chain is `RenderPass` → `UnrealBloomPass` (strength 0.5, threshold 0.74, so only the Sun, lightning, and bright cloud crests bloom) → `GammaCorrectionShader` for correct sRGB output → `FXAAShader` (composer render targets lose MSAA, so FXAA restores edge smoothing) → a custom cinematic pass adding vignette and animated film grain. A `THREE.Points` crystal storm (≈5 200 additive particles on high quality) rains slowly through the full altitude column and wraps around, so the probe visibly falls through suspended ice. The composer is skipped on `low` quality or when the CDN modules are unavailable, falling back to direct rendering; resize, pixel-ratio relief, and photo capture all route through the composer when present.
- A real Galileo orbiter model (NASA 3D Resources) hangs overhead at the start of the descent — the spacecraft that released the probe in 1995 — slowly rotating and receding as the visitor sinks.
