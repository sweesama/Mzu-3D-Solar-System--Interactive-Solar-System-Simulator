    ...
    root.createMoonDiscoveries = function ({ scene, camera, surface, rock, getWalker, isExploring, isPhotoMode, clearMovement, onPhoto, onDiscover, getNotes, language: initialLanguage, expedition, terrain, strings }) {
        ...
        const copy = strings || {
            en: {
                ...
                teasers: ['Find a place on the crater rim and read the landscape.', 'Approach the large boulder, then turn it around in the specimen viewer.', 'Reach the viewing point and find Earth above the horizon.', 'Walk south from arrival to find a lunar module resting on the plain.', 'Hike to the eastern rim of the deep crater and look into its shadowed floor.'],
                details: ['The rim is built from material displaced during impact. The surrounding debris is called ejecta. Our crater is an artistic example of this process, not a reconstruction of a named lunar crater.', 'Lunar regolith is a mixture of rock fragments, mineral grains, and glass produced by impacts. Shape alone cannot tell us this model’s mineral composition or age. Rotate the representative rock to examine its surface.', 'Earth is much larger in the lunar sky than the Moon is in our sky. Tidal locking keeps it near the same place when viewed from much of the near side, although libration makes it move somewhat. This scene uses an artistic sky arrangement.', 'Six Apollo lunar modules landed between 1969 and 1972; their descent stages never left the Moon. This model comes from NASA\u2019s open 3D model archive. Its placement on this plain is artistic, not a surveyed landing site.', 'Permanently shadowed craters near the lunar poles are among the coldest places in the Solar System — colder than −230 °C. Ice delivered by comets may survive there for ages. This hollow is a modest artistic echo of those places.']
            },
            zh: {
                ...
                teasers: ['走到环形山边缘，读一读地形留下的痕迹。', '走近那块大岩石，在查看器中转动并观察它。', '到达眺望点，寻找地平线上方的地球。', '从抵达点向南走，找一台停在平原上的登月舱。', '徒步到东侧深坑的边缘，俯视它阴影中的坑底。'],
                details: ['撞击将物质挤压、抛散，堆成隆起的坑缘，周边散落的碎屑称为溅射物。这里的环形山是这一过程的艺术示例，并不对应某个真实命名的陨石坑。', '月壤由岩石碎屑、矿物颗粒以及撞击产生的玻璃等组成。仅凭外形无法判断这个模型的矿物成分或年龄。可以旋转这块代表性岩石，仔细观察表面。', '从月球看地球，其视直径比从地球看月球大得多。潮汐锁定使月球正面许多地区天空中的地球位置相对稳定，天平动仍会带来一定变化。本场景的天空经过艺术构图。', '1969 至 1972 年间，六艘阿波罗登月舱先后降落，它们的下降级从此再未离开月球。这台模型来自 NASA 开放三维模型库，摆放位置是艺术创作，并非真实着陆点。', '月球两极的永久阴影坑是太阳系中最冷的地方之一，可低于零下 230 摄氏度，彗星带来的水冰或许能在其中留存很久。这处浅坑是对那些地方的朴素艺术再现。']
            }
        };
