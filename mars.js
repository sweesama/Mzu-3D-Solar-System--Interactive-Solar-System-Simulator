    ...
    function buildRocks(texture, rockModels) {
        ...
        for (let group = 0; group < groups; group++) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                if (model) {
                    const s = size * 1.9 * (ventifact ? 1.3 : 1);
                    const lift = -model.geometry.boundingBox.min.y * s;
                    transform.position.set(x, MarsTerrain.sampleSurface(surface, x, z) + lift * 0.38, z);
                    transform.scale.set(s * (ventifact ? 1.3 : 1), s * 0.9, s);
                    transform.rotation.set((rand() - 0.5) * 0.5, ventifact ? -0.9 + (rand() - 0.5) * 0.4 : rand() * Math.PI * 2, (rand() - 0.5) * 0.5);
        ...
        for (const [x, z, size] of heroes) {
            ...
            if (model) {
                ...
                const lift = -model.geometry.boundingBox.min.y * s;
                rock.position.set(x, MarsTerrain.sampleSurface(surface, x, z) + lift * 0.38, z);
            } else {
