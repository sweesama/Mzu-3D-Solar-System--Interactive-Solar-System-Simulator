(function (root) {
    'use strict';
    // Uranus ring-flight audio: near-silent vacuum hum, occasional ice pings off the hull,
    // faint radio hiss, and a thruster whoosh. Same interface as the other planet audio modules.
    function create() {
        ...
        function start() {
            ...
            if (!context) {
                ...
                // Spacecraft hum -?a quiet filtered hiss, far softer than Jupiter's wind.
        ...
        return {
            ...
            setDescent(rate) {
                ...
                const level = Math.min(0.04, 0.006 + Math.abs(rate) * 0.008);
                humGain.gain.setTargetAtTime(level, context.currentTime, 0.3);
                humFilter.frequency.setTargetAtTime(140 + Math.abs(rate) * 90, context.currentTime, 0.35);
            },
            step(intensity = 1) {
                if (!ready()) return;
    ...
    root.UranusAudio = Object.freeze({ create });
